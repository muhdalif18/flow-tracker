...
- **flows**: `id`, `name`, `description`, `group_name` (sidebar grouping), `order_idx`, `created_by`, `copy_enabled` (boolean, admin-controlled)
...
| PUT | `/api/flows/:id/copy-enabled` | Toggle `copy_enabled` flag (admin only) |
| POST | `/api/flows/:id/copy` | Copy flow with all modules/scenarios/steps (requires `copy_enabled = true`) |
...
Admins can enable copying for any flow via `PUT /api/flows/:id/copy-enabled`. Once enabled, any authenticated user can copy the flow via `POST /api/flows/:id/copy`. The copy operation:
- Creates a new flow named `[Original Name] (Copy)` owned by the copying user
- Copies all modules, scenarios, and steps with their structure intact
- Copies only `description` and `expected` from steps — no execution data (status, evidence, remarks, etc.)
- Sets `copy_enabled = false` on the new flow
