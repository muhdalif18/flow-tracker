...
const IcoCheck = () => <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 8.5l3 3 7-7"/></svg>;
const IcoX     = () => <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M4 4l8 8M12 4l-8 8"/></svg>;
const IcoDash  = () => <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M4 8h8"/></svg>;
const IcoPlus  = () => <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M8 3v10M3 8h10"/></svg>;
const IcoUp    = () => <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 12V4M4 8l4-4 4 4"/></svg>;
const IcoDown  = () => <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 4v8M4 8l4 4 4-4"/></svg>;
const IcoTrash = () => <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5h10M6 5V3h4v2M5 5l.5 9h5L11 5"/></svg>;
const IcoLock  = () => <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4"><rect x="3.5" y="7" width="9" height="6" rx="1"/><path d="M5.5 7V5a2.5 2.5 0 115 0v2"/></svg>;
const IcoDoc   = () => <svg width="18" height="18" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"><path d="M4 2h6l3 3v9H4V2z"/><path d="M10 2v3h3M6 8h5M6 11h5"/></svg>;
...
function AddScenarioModal({ moduleId, onClose }: { moduleId: string; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    await addScenario(moduleId, { blid: blid.trim(), description: desc.trim() });
...
function StepCard({ step, stepNo, canEdit }: { step: TestStep; stepNo: number; canEdit: boolean }) {
  ...
  const mark = (status: TestStep['status']) => {
    ...
    if ((status === 'pass' || status === 'fail') && !step.date_tested) d.date_tested = today();
...
function ExpandPanel({ sc, canEdit }: { sc: Scenario; canEdit: boolean }) {
  ...
  const handleAddStep = async (e: React.FormEvent) => {
    ...
    await addStep(sc.id, { description: stepDesc.trim(), expected: stepExp.trim() });
    setStepDesc(''); setStepExp(''); setShowAdd(false); setAdding(false);
  };
    ...
    <div className="expand-panel">
      ...
      {canEdit && (showAdd ? (
        <form className="add-step-form" onSubmit={handleAddStep}>
          ...
          />
          <textarea
            value={stepExp} onChange={e => setStepExp(e.target.value)}
            placeholder="Expected result *" rows={2} required
          />
          <div className="add-step-btns">
            <button type="button" onClick={() => { setShowAdd(false); setStepDesc(''); setStepExp(''); }}>Cancel</button>
            <button type="submit" className="btn-primary" disabled={adding}>{adding ? 'Adding…' : 'Add Step'}</button>
          </div>
        </form>
      ) : (
        sc.steps.length > 0 && (
...
function ScenarioRow({ sc, canEdit }: { sc: Scenario; canEdit: boolean }) {
  ...
  const testedStep = [...sc.steps].reverse().find(s => s.status !== 'untested');
  const metaDate = testedStep?.date_tested || '';
  const metaAdo  = testedStep?.ado_ticket  || '';

  return (
    <>
      <tr className={`sc-row ${derived === 'pass' ? 'row-pass' : derived === 'fail' ? 'row-fail' : ''}`}>
        {/* BLID */}
        ...
        <td>
          <div className="sc-desc">
            {sc.description}
            {issue && (
              <span className={`sc-issue ${issue}`}>{issue}</span>
            )}
          </div>
          {sc.steps.length > 0 && (
            <div className="step-count-badge">
              {sc.steps.filter(s => s.status === 'pass').length > 0 && (
                <span style={{ color: 'var(--ok)' }}>{sc.steps.filter(s => s.status === 'pass').length}✓</span>
              )}
              {sc.steps.filter(s => s.status === 'fail').length > 0 && (
                <span style={{ color: 'var(--bad)' }}> {sc.steps.filter(s => s.status === 'fail').length}✗</span>
              )}
        ...
        <td style={{ textAlign: 'center' }}>
          {derived === 'pass'     && <span className="sst-pill sst-pill-pass"><IcoCheck />PASS</span>}
          {derived === 'fail'     && <span className="sst-pill sst-pill-fail"><IcoX />FAIL</span>}
          {derived === 'untested' && <span className="sst-pill sst-pill-nt"><IcoDash />N/T</span>}
        </td>
        ...
        <td>
          <div className="sc-meta">
            ...
            {metaAdo
              ? <span className="sc-meta-ado">{metaAdo.startsWith('http') ? '🔗 link' : metaAdo}</span>
        ...
        {/* Expand chevron */}
        <td style={{ textAlign: 'center' }}>
          <button className={`sc-chev-btn ${isExp ? 'open' : ''}`} onClick={() => toggleExpand(sc.id)}>
            <IcoChev />
          </button>
        </td>
      </tr>
...
function ModuleCard({ mod, flow }: { mod: Module; flow: Flow }) {
  ...
  const borderC: Record<string, string> = {
    complete: '#059669', blocked: '#dc2626', major: '#d97706',
    minor: '#d97706', progress: '#3b82f6', pending: '#e2e8f0', empty: '#e2e8f0',
  };
  ...
  const sideBg = mod.side === 'eDS' ? 'rgba(29,78,216,.1)' : 'rgba(124,58,237,.1)';
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={e => { if ((e.target as HTMLElement).closest('button')) return; setCollapsed(c => !c); }}
        style={{ cursor: 'pointer' }}
      >
        {/* Left: chevron + badge + title */}
        <div className="mod-hdr-left" style={{ gap: 10 }}>
          <span className="mod-chev"><IcoChev /></span>

          <span className="mod-badge" style={{ background: sideBg, color: sideC, borderColor: sideBd }}>
            {mod.label}
          </span>
          ...
          <div>
            <div className="mod-name">
              {mod.name}
              <span style={{ fontSize: 10, fontWeight: 600, color: sideC, marginLeft: 7, textTransform: 'uppercase', letterSpacing: '.06em', opacity: .8 }}>
                {mod.side}
              </span>
            </div>
        ...
        <div className="mod-hdr-right" style={{ gap: 10 }}>
          ...
          <div className="mini-ct">
            <div className="mct mct-p"><span className="mct-dot" />{ms.pass}</div>
            <div className="mct mct-f"><span className="mct-dot" />{ms.fail}</div>
            <div className="mct mct-u"><span className="mct-dot" />{ms.untested}</div>
          </div>
      ...
      {gated && (
        <div className="gate-banner-v2">
          <IcoLock />
          <span><strong>Gated</strong> — a previous module has a Blocker issue. Resolve it before testing here.</span>
        </div>
      )}
...
export function ScenariosView() {
  ...
  return (
    <div>
      {activeFlow.modules.map(mod => <ModuleCard key={mod.id} mod={mod} flow={activeFlow} />)}
    </div>
  );
}
